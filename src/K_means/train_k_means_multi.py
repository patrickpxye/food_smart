...
for cluster_id, center in cluster_centers.items():
    ...
    # Annotating the cluster center with the most common labels
    label = ', '.join(most_common_labels[cluster_id])  # Join labels into a single string
    plt.annotate(label, (center[0], center[1]), textcoords="offset points", xytext=(0,10), ha='center')

plt.title(f'K-Means Clustering with t-SNE visualization, multi-label, top {TOP_N} labels')
...
for idx, center in enumerate(reduced_centers):
    ...
    if cluster_points.size > 0:
        ...
        label = ', '.join(most_common_labels[cluster_id]) 
...
plt.title(f'Ingredient Clusters Visualization, multi-label, top {TOP_N} labels')
plt.xlabel('t-SNE feature 1')
plt.ylabel('t-SNE feature 2')
# plt.legend()
plt.show()
